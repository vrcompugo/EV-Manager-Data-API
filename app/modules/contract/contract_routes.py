...
from .contract_services import add_item, update_item, get_items, get_one_item


api = Namespace('Contract')
_item_input = api.model("Contract_", model={
...
class Items(Resource):
    ...
    @token_required("create_contract")
    def post(self):
        """Creates a new User """
        data = request.json
...
class User(Resource):
    ...
    @api.doc('update contract')
    @api.expect(_item_input, validate=True)
    @token_required("update_contract")
    def put(self, id):
        data = request.json
