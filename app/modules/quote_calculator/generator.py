...
def generate_contract_summary_pdf(lead_id, data):
    ...
    if datetime.datetime.now() >= datetime.datetime(2021,12,14,0,0,0):
        if "data" in data and data["data"].get("cloud_quote_type") in ["synergy"]:
            add_pdf_by_drive_id(merger, 7801067, cached=True)  # https://keso.bitrix24.de/disk/downloadFile/7801067/?&ncc=1&filename=contractingvertrag_synergie360_2023_formular2.pdf
        else:
